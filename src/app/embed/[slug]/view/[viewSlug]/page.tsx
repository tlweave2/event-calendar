...
export default async function ViewPage({
  ...
  const view = await prisma.calendarView.findFirst({
    where: { tenantId: tenant.id, slug: viewSlug },
  });
  if (!view) notFound();

  void recordPageView(tenant.id, "calendar");

  const allEvents = await getApprovedEvents(tenant.id);
  const events = allEvents.filter(
    (e) => e.categoryId && view.categoryIds.includes(e.categoryId)
  );
  ...
  const viewCategories = tenant.categories.filter((category) =>
    view.categoryIds.includes(category.id)
  );
